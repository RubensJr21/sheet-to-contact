...
const InputByTextArea = () => {
    ...
    <>
        <div className="w-75 h-100 mt-2 mb-4 shadow position-relative">
            <textarea className="form-control w-100 h-100 shadow" style={{resize: "none"}} placeholder="Cole o csv aqui.." id="insertionCSV" value={inputCsv} onChange={onTextAreaChange}/>
            <button type="button" onClick={onClick}className="btn btn-success position-absolute bottom-0 end-0 me-1 mb-1">Carregar</button>
        </div>
    </>
    )
}

export default InputByTextArea;